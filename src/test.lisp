...
(defun load-ref-image/png-read (file)
  (let* ((png (png-read:read-png-file file))
    ...
    ;; workaround for sbcl bug affecting opticl
    (when (or (typep (png-read:image-data png)
                     '(simple-array t))
              (typep (png-read:image-data png)
                     '(simple-array bit))
              (typep (png-read:image-data png)
                     '(simple-array (unsigned-byte 2)))
              (typep (png-read:image-data png)
                     '(simple-array (unsigned-byte 4))))
      (let ((i (make-array (array-dimensions (png-read:image-data png))
                           :element-type
                           (if (< (png-read:bit-depth png) 8)
                               '(unsigned-byte 8)
                               '(unsigned-byte 16)))))
        (loop for x below (array-total-size (png-read:image-data png))
              do (setf (row-major-aref i x)
                       (row-major-aref (png-read:image-data png) x)))
        (setf (png-read:image-data png) i)))

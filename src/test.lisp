...
(defun load-ref-image/png-read (file)
  (let ((png (png-read:read-png-file file)))
    (when (png-read:transparency png)
               ...
               do (loop for y below (second dims)
                        do (loop for i below (1+ (or c 1))
                                 do (setf (aref new x y i)
                                          (or (old x y i)
                                              (aref (png-read:transparency png)
                                                    x y)))))))
        (setf (png-read:image-data png) new)))
    (opticl:transpose-image (png-read:image-data png))))

(defun load-ref-image/cl-png (file)
  (let ((png (png::decode-file file)))
    (when (= 1 (array-dimension png 2))
      (let ((2d (make-array (list (array-dimension png 0)
                                  (array-dimension png 1))
                            :element-type (array-element-type png))))
        (loop for i below (array-total-size png)
              do (setf (row-major-aref 2d i)
                       (row-major-aref png i)))
        (setf png 2d)))
    png))
...
(defun load-ref-image (file)
  (ecase *ref*
    ...
    (:cl-png (load-ref-image/cl-png file))))
  ...
  (let ((*failed*)
    ...
    (flet ((test-image (file)
             (multiple-value-bind (image error)
               ...
               (when (and image (eql *ref* :cl-png))
                 (pngload::expand-gray image))
               (let ((ref (load-ref-image file)))
                 (when flip
                   (setf ref (opticl:vertical-flip-image ref)))
                 (when flatten
                   (setf ref
                         (make-array (array-total-size ref)
